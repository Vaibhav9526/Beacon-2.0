...
CREATE TABLE IF NOT EXISTS community_bans (id text PRIMARY KEY, community_id text NOT NULL REFERENCES communities(id) ON DELETE CASCADE, citizen_id text NOT NULL REFERENCES citizens(id) ON DELETE CASCADE, reason text NOT NULL, actor_id text NOT NULL, created_at timestamptz NOT NULL DEFAULT now());
CREATE UNIQUE INDEX IF NOT EXISTS community_ban_member ON community_bans(community_id, citizen_id);
...
CREATE INDEX IF NOT EXISTS reports_incident_created_idx ON reports(incident_id, created_at DESC);
CREATE INDEX IF NOT EXISTS analysis_incident_created_idx ON analysis_runs(incident_id, created_at DESC);
CREATE INDEX IF NOT EXISTS messages_community_created_idx ON messages(community_id, created_at);
CREATE INDEX IF NOT EXISTS messages_community_sender_idx ON messages(community_id, sender_id);
CREATE INDEX IF NOT EXISTS messages_community_status_created_idx ON messages(community_id, moderation_status, created_at);
CREATE INDEX IF NOT EXISTS assignments_incident_idx ON assignments(incident_id, created_at DESC);
CREATE INDEX IF NOT EXISTS assignments_sos_idx ON assignments(sos_id, created_at DESC);
CREATE INDEX IF NOT EXISTS sos_citizen_status_created_idx ON sos_requests(citizen_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS communities_status_created_idx ON communities(status, created_at DESC);
CREATE INDEX IF NOT EXISTS audit_events_created_idx ON audit_events(created_at DESC);
CREATE INDEX IF NOT EXISTS delivery_attempts_created_idx ON delivery_attempts(created_at DESC);
CREATE INDEX IF NOT EXISTS incidents_trust_created_idx ON incidents(trust_state, created_at DESC);
